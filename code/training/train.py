...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if has_yolo_s3_data_dir and opt.upload_dataset:
        raise ValueError("'--upload_dataset' argument cannot be passed when \
            'yolo_s3_data_dir' field is not empty in 'data.yaml'.")
    ...
    if RANK in [-1, 0]:
        loggers = Loggers(save_dir, weights, opt, hyp, \
            LOGGER, mmdet_keys=opt.mmdet_tags, class_names=names)  # loggers instance
        if loggers.wandb:
    ...
    if pretrained:
        ...
        if epochs < start_epoch:
            # LOGGER.info(f"{weights} has been trained for {ckpt['epoch']} epochs. Fine-tuning for {epochs}
            #  more epochs.")
        ...
        if RANK in [-1, 0]:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if opt.s3_upload_dir:
                    s3_file = "s3://" + \
                        str(Path(opt.s3_upload_dir.replace("s3://","")) / save_dir.name / "weights" / "best.pt")\
                            .replace(os.sep, '/')
...
def parse_opt(known=False):
    ...
    # AWS arguments
    parser.add_argument('--s3_upload_dir', type=str, default=None, \
